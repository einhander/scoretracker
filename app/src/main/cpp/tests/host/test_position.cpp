...
void test_position_exact(){auto r=makeRef();auto o=DtwMatcher(r).global(makeLive(r));CHECK(o.valid||o.matchQuality>0);CHECK_NEAR(o.quarterBeatPosition,r.frames()[9].quarterBeatPosition,.5);}
void test_position_stretch(){auto r=makeRef();for(float ratio:{.8f,1.2f}){auto o=DtwMatcher(r).global(resample(r,ratio));CHECK(o.quarterBeatPosition>r.frames()[4].quarterBeatPosition);CHECK(o.matchQuality>.5f);}}
